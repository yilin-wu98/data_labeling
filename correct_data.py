...
def is_increasing(lst):
    ...
    for i in range(len(lst) - 1):
        # If any element is greater than its successor, the list is not increasing
        if lst[i] > lst[i + 1]:
            return False
    # If the loop completes without finding a violation, the list is increasing
    return True
def main():
    ...
    for idx,filename in enumerate(sorted(os.listdir(output_folder))[3841:]):
  
        ## get the episode index from the filename
        episode_index = filename.split('_')[1]
        task_index = int(filename.split('_')[3])
        episode_num = 0
        # if task_index in [7,8,19, 29, 54, 61]:
            # episode_num += 1
        ...
        if task_index == 31:
            ...
            data, plans, frames = load_annotations_txt(os.path.join(output_folder, filename)) 
            if len(plans) != 3:
                print("plans:", plans)
                print("frames:", frames)
    #     ## check if the frames elements are in increasing order
    #     if not is_increasing(frames):
    #         print(f"Frames in {filename} are not in increasing order")
    #         print(frames)
    #         sorted_frames = sorted(frames)
    #         print("Sorted frames:", sorted_frames)
    #         modify_plans(data, sorted_frames, filename)
